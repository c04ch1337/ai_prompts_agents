...
**Onboarding flag:** KB-06 key `sola_grace_onboarding_complete` (timestamp) marks onboarding complete. When absent, the UI shows the Phoenix Marie onboarding overlay.
...
- **Trigger:** `GET /api/v1/onboarding/status` returns `needs_onboarding: true` when KB-06 has no `sola_grace_onboarding_complete` and chat history is empty.
...
- **Identity:** `PAGI_IDENTITY_NAME=Phoenix Marie`, `PAGI_IDENTITY_MODE=Sovereign_Companion`, `PAGI_VERBOSITY=Candid`.
